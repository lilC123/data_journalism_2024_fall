...
va_ev_pres_24 <- clean_names(read_csv("data/va_ev_pres_24.csv")) #VA early voting for the presidential election of 2024
```
...
md_ev_pres_16 <- clean_names(read_csv("data/md_ev_pres_16.csv")) |>
  ...
  summarise(
    total_16=sum(total, na.rm=TRUE)
  ) #finding total EV for 2016, same code as Q1
...
md_ev_pres_20 <- clean_names(read_csv("data/md_ev_pres_20.csv")) |>
  ...
  summarise(
    total_20=sum(total, na.rm=TRUE)
  ) #finding total EV for 2020, same code as Q1
...
md_ev_pres_24 <- clean_names(read_csv("data/md_ev_pres_24.csv")) |>
  ...
  summarise(
    total_24=sum(total, na.rm=TRUE)
  ) #finding total EV for 2024, same code as Q1

md_ev_pres_combined <- md_ev_pres_24 |>
  left_join(md_ev_pres_20, join_by(county_name)) |>
  left_join(md_ev_pres_16, join_by(county_name)) |> #join all the dataframes at once because they all have the same structure, same as Q1
  arrange(desc(total_20))

ggplot(md_ev_pres_combined, aes(x=total_24, y=total_20, color=total_16)) +
  geom_point() #couldn't find a great way to visualize three variables, so a plot chart with color coded values was the best we could do
```
...
```{r}
md_early_voting_support <- clean_names(read_csv("data/md2024evbyparty.csv"))

md_early_voting_support |>
    ggplot() +
  geom_bar(aes(x=x1, weight=total_votes, fill=total_votes))+labs(x = "Party Affiliation")

sd_early_voting_support <- clean_names(read_csv("data/sd2024evbyparty.csv"))
