...
Several of the recipients are split up and treated as separate, simply because the spellings are slightly different for each one. Something as simple as a letter being lowercase instead of uppercase in the same spelling is causing the code to treat them as two separate recipients.
...
The refinr package changed 'SCARLET OAK' to 'Scarlet Oak'. This is the only change made in the first 20 rows.
...
The refinr package condensed the "W. MILLAR & CO." recipient, the "W. MILLAR & CO. CATERING" recipient, and the "WASHINGTON HILTON" recipient.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer**

No, the changes it made do not vary substantially from the key_collision_merge function.
...
```{r}
dc_food_refined <- read.csv("data/dc_food_cleaned_csv.txt")
```

```{r}
dc_food_refined_by_recipient <- dc_food_refined |>
  group_by(recipient_name) |>
  summarise(
    count=n()
  ) |>
