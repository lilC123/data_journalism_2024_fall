...
**Answer** This codeblock is adding a new column to the dataset called "percent_election_day" which is made up of the election day votes divided by the total number of votes.
...
**Answer** The table is now arranged by the new column we created, "percent_election_day", in descending order. The counties with the highest percentage of election day votes against total votes will appear at the top.
...
**Answer** The answer is neither more intersting nor more useful, since it is the exact same data as the previous codeblock, just flipped.
...
**Answer** "str_to_lower()" converts everything to lowercase, "str_to_title()" capitalizes the first letter of every word.
...
**Answer** The mutate statement is categorizing the contributions into standard English. It looks at the number in the "amount" column, and puts it into a category whose name matches the range it falls into. These category names are in our new created column called "amount_category".
...
**Answer** "Less than $100" is the largest category by dollar amount.
...
**Answer** With the new variable, we can call this condensed form of the data while also still having the complete dataset in another variable if/when we need to call it.
...
**Answer:** The dates changed from m/d/y format to y-m-d format thanks to the "mdy()" function.
...
**Answer** The function "floor_date()" pulls every date down to the earliest possible date within the constraints. Each month is now represented by its first day, so '2022-06-01' represents June. The total amount column shows how many total expedentirues in terms of dollars took place during that month.
