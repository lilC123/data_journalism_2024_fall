...
The first line says: "One or more parsing issues, call `problems()` on your data frame for details, e.g.:
  dat <- vroom(...)
  problems(dat)"
...
There seem to be 1,640 problems in the results data. Upon looking at the data, specifically in the rows listed as having a problem, I cannot figure out what the issue is with these particular rows. It may have to do with the columns titled "provisional" and "limited", as those are columns 12 and 13, which seem to be where the problems are occurring.
...
It does not return any Warning or apparent parsing errors.
...
"district" is a <dbl> data type, indicating a number of some sort. "precinct" is a <chr> or 'character' type and "mail" is also a <dbl> type.
...
When mail is NA, the count sits at 402,345. I think this means that the vast majority of votes were not mailed in.
...
There were 12 registrants in January 1900, according to the data. I think the first 20 results suggest that many people register to vote in January at the beginning of the year.
...
I could maybe see investigating the 1,435 for "CONFIRMATION NOT RETURNED" and 574 for "CONFIRMATION RETURNED UNDELIVERABLE". That's more than 2,000 voters who registered but had to re-register. What about the people that never re-registered? In a county with only 28,456 registrants, 2,000+ is kind of a biggish number to not account for.
...
"amount" is a <chr> or 'character' data type.
...
**Answer** Firstly, the "amount" column is a character data type when it should be a number. This makes it harder to do math with that column since it is not recognized as a numeric value. Also, There’s a fully duplicated row – a common problem in data sets. The 14th row is exactly the same as the seventh. Also, Derrick Hamilton's information is duplicated, as the data counts him in two separate rows. Lastly, the data is inconsistent in several ways, including multiple zip code variations (5-digit vs. 9-digit) and different versions of the same street (Merry Knoll Lane vs. Merry Knoll Ln), making grouping more difficult.
...
**Answer** The function adds an "x" to the beginning of the "1_linenumber" column, though I am not sure why.
...
The error says: "Error in `summarise()`: In argument: `total_amount = sum(amount)`. Caused by error in `sum()`: invalid 'type' (character) of argument" I think this means that when the code tried to add values to get the sum, it couldn't do so because the values were of the 'character' data type instead of numbers (<dbl>).
...
The new data type is 'double' or <dbl>.
...
**Answer** The total contributions in this data set is 226.
...
There is one duplicate of Derrick Hamilton's data.
...
In the new dataframe, all of the zip codes have been standardized to 5-digit format, and they are all the same (21918).
...
2 misspellings of Conowingo still exist in the data.
...
1 misspelling of Conowingo still exists in the data.
...
0 misspellings of Conowingo still exist in the data.
