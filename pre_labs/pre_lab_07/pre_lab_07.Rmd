...
```{r echo=FALSE}
census_api_key("661f4f0161698ffa503fad7040f22297b2e052d0", install=TRUE)
```
...
The state with the highest percentage growth is Nevada, winning by a wide margin at 35% (next closest is 24.5%).
...
Is the difference between Calvert and Montgomery significant? **Answer** No, as the bars for these counties overlap.

Is the difference between Howard and everyone else significant? **Answer** Yes, as Howard's bar does not come close to any other.
...
It does not appear that any county saw their median incomes drop from 2010 to 2020.
...
**Answer** Zip code 21202 seems to be the only zip code in the top 20 with an estimated median household income below of $50,000.
...
head(md_zips_with_acs)
```

**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? **Answer** What jumps out to me is that 20816, 20812, and 20817 are all in the top 6. Assuming that these zip codes are all in the same general area of Maryland, what do the demographics of this area look like?
