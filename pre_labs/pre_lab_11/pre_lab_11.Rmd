...
I notice that the columns would not be fun to run code with as they contain different casings of letters, as well as things like periods, commas, and parentheses.
...
The numbers are referring to the column, i.e. Harris was the second column so the number 2 is correct.
...
The output suggests there are 55 <li> tags in the HTML source.
...
We get regular words for each <li> tag as opposed to the associated HTML language that makes it harder to identify each text.
...
The output is a one-column dataframe of committee names.
...
Now we're getting closer. The rows in this dataframe we don't want to begin with a backslash. Let's get rid of those using `filter()`:
