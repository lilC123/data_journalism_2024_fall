...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** Outside of a slight difference in the order of the columns ("UNA" and "OTH" swap back and forth), the CSV files all have the same structure.
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows, which is the correct amount.
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** The message says that it is joining using the same "join_by()" function we included earlier, since "COUNTY" seems to be the only column the two datasets have in common.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's County is the answer to both
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard county has the highest percentage of Asian people among its 18+ population, yet there are 5 counties that are more Democratic than Howard.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** The datatype is <dbl> or doubles.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** The datatype is now <chr> or character.
