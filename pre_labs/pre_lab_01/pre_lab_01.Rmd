...
**131395 observations and 24 variables**
...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **The "last_name" column data type is <chr> which is character, and "amount" is <dbl> which means it is a number with decimal points. I would guess that the "last_name" column represent the last names of donors, while "amount" represents the monetary value of each donation.**
...
**The code seems to be counting how many contributions were made on each date. The answer is laid out in ascending order in the date column, not by number of contributions. The 'head()' function only returns the first couple rows of the data, but doesn't seem to be in an order that is helpful to us.**
...
**Instead, the dated contributions are now arranged by the dates with the highest contribution count in descending order.**
...
**We changed what column we are focused on, swapping out 'date' for 'memo_text' and 'amount'. This now shows the most common contribution amounts and destinations.**
...
**This codeblock now counts exactly how much money each campaign received in contributions, as well as how many different contributions they each received.**
...
**We now have more descriptive summarization stats for the contributions to each campaign, such as mean and median. From this data, it looks like Donald Trump received the most support in Maryland in terms of financial contributions.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **Instead of grouping by contribution destination, we now look at their origin, seeing which cities in Maryland were most active in terms of making contributions. The data is sorted by total amount donated in descending order, putting Annapolis on top.**
