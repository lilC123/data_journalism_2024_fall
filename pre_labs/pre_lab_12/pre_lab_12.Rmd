...
The dataframe is four columns each with information relating to the press releases. This includes their date, title, url, and text within.
...
The unique() function reduces the list to only having one stance of each unique word, so there are no copies or duplicates. Although, capital letters are treated as unique even if it is the same word as another in the list.
...
The output is a dataframe of every single word in the text, with each word having their own row in the solo "word" column.
...
The graphic shows the most commonly found/used words in the press releases. It is not particularly surprising or therefore, interesting as the most common words are "the", "and", and "to".
...
It is much more intersting because the words are actually meaningful. It seems that "health" and "support" are the leaders.
...
The word "support" is in second place for both years, while the terms "baltimore" and "funding" are also consistently high.
...
**Task** Run the following code and describe the results. **Answer**
The phrase "amount included" is the leading pair of words with "health care" and "prince george's" trailing.
...
**Task** Run the following code and describe the results. **Answer**
The phrase "health care" is the leading pair of words with "amount awarded" and "purpose funds" tied for second.
...
The lemmatize_words() function might be more useful because ultimately, the words in the stem_words() output kind of all mean the same thing. You can conclude that the topic of driving is prevalent in the list pretty easily without having to sort through variations of the same word.
...
lemmatize_words() is more useful. We can see that it combines "funding" and "funds" instead of treating them as two different words. While they are, technically, they are the same idea and should be considered the same usage.
...
Based on the 20-or-so words I looked over, the sentiments all seem like they make sense. I unfortunately won't be checking all 1,100+ entries.
