...
I see a pretty normal-looking glimpse function with a lot of random numbers. There are some columns such as "Shape_Length" and "Shape_Area" which are confusing. There are only 10 columns total.
...
The dataframe contains information for each voting precinct in Prince George's County, evidenced by the fact that each "PRECINCT_NAME" value is a school or community center.
...
The output is a map of Prince George's County, broken down by voter precinct. We also see the latitutde and longitude lines on the map, with the values acting as the X and Y axes.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
There is no output as all the code did was load the Tigris library.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The dataframe looks similar to the previous one, it just has a few more columns. It also has the same multipolygon dataype at the end.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It is the same dataframe except now there are only 24 rows compared to 3,235.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is a map of the state of Maryland with county borders.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the same map as before except all the precincts are filled in gray as opposed to having a select few be blank.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The dataframe is a breakdown of voter turnout versus eligibility for each precinct in Prince George's County.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The new dataframe is a combination of the turnout dataframe with the precincts dataframe in Prince George's County.

```{r}
pg_precincts_with_turnout <- pg_precincts |> left_join(pg_turnout, by=c("PRECINCT_ID"="precinct"))
glimpse(pg_precincts_with_turnout)
```
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the same Prince George's COunty precincts map as before, except now each precinct has a color value depending on how many eligible voters it had. The higher numbers are lighter blue while the lower numbers are darker blue.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is a dataframe of voters broken down by party for each county in the state of Maryland.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is an estimate of the population for each county in the state of Maryland.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is a dataframe that joins our population and voters dataframes while also adding the multipolygon column at the end.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is just the voters by county dataframe.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the same population dataframe as before, except the county column is now cleaned to be just the name of the county and nothing else.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is a combined dataframe of the popluation and voters dataframes.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the same population dataframe, except the names of 'St. Mary's' and 'Baltimore' now match the voter dataframe.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the same joined dataframe as before, except now there is a new column that shows the number of voters per 10,000 people.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is a map of the state of Maryland with county borders. Counties with a higher number of voters per 10,000 are lighter blue and lower numbers are darker blue.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the same map, except now the changes in color are much more drastic so the differences are easier to identify.
...
The output is the exact same map for me. I notice that both extremes of the state are right next to each other, which I would not have predicted.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the same joined dataframe as before, except now there is a new column showing the amount of unaffiliated voters per 10,000.
...
The output is the same MD county map with new colors, as this one shows unaffiliated voters per 10,000. We see that there are more extremes on either side of the scale.
