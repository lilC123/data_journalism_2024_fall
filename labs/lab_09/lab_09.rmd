...
**A1.** I think the biggest story here is how mail-in votes soared for the 2020 Presidential election despite clearly being the least common option in the elections both before and after in 2018 and 2022. This is likely due to the Covid pandemic, as we can see that the 2020 election was unique.
URL: https://datawrapper.dwcdn.net/dsDiu/2/

```{r}
md_votes_by_type <- clean_names(read_csv("data/md_votes_by_type.csv")) |>
  mutate(pct_early=early/total*100) |>
  mutate(pct_elec_day=election_day/total*100) |>
...
**A2.** I picked the CSV about turnout for female voters in Early Voting because the random number generator that I consulted chose number 6.
URL: https://datawrapper.dwcdn.net/anqOZ/1/

```{r}
ev_totals_women <- clean_names(read_csv("data/ev_totals_women.csv")) |>
...
write_csv(ev_totals_women, "data/ev_totals_women.csv")
...
**A3.** For the ActBlue donations, the most interesting story is that Montgomery County is dominating everywhere else with pretty much all of the high-donation zip codes coming within MoCo. As for the WinRed donations, 41,865 of the donations come from zip code 21740 which is Hagerstown, while the next highest donation count is zip code 20854 (Potomac) with 12,131 donations.
ActBlue URL: https://datawrapper.dwcdn.net/5wD9C/1/        WinRed URL: https://datawrapper.dwcdn.net/d6LJT/1/
...
md_winred_zip_totals <- clean_names(read_csv("data/md_winred_zip_totals.csv")) |>
  ...
  mutate(per_capita=donations/estimate*1000) |>
  select(zip, donations, estimate, moe, per_capita) |>
